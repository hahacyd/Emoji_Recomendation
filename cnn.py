...
from kaggle_preprocessing import getCnn_Data, getEmbed_lookup, getTarget
...
class CNN(nn.Module):
    ...
    def __init__(self, embed_model, vocab_size, output_size, embedding_dim,
        ...
        self.embedding.weight = nn.Parameter(
            torch.from_numpy(embed_model.vectors))  # all vectors
    ...
    def forward(self, x):
        ...
        conv_results = [self.conv_and_pool(
            embeds, conv) for conv in self.conv2d]
...
net = CNN(embed_lookup, vocab_size, output_size,
          embedding_dim, kernel_sizes=kernel_sizes)
    ...
    for epoch in range(train_epoch):
        ...
        for batch_index, (inputs, labels) in enumerate(data_loader, 0):
            ...
            if batch_index != 0 and batch_index % 100 == 0:
                print("epoch = %d [ %d/%d ] loss = %f" % (epoch, batch_index / intervel, len(
...
def test_CNN():
    ...
    dataset = Data.TensorDataset(torch.from_numpy(
        testX).long(), torch.from_numpy(testy))
